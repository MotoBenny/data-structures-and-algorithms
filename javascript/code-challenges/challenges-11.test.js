...
const count = (target, input) => {
  ...
  let count = 0;
  input.forEach((arr) => {
    count += arr.filter(elem => elem === target).length;
  });
  return count;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArr = [];
  input.forEach(index => {
    let test = index.reduce((newVal, currVal) => {
      if (currVal % 5 === 0 && typeof (currVal) === 'number') {
        newVal.push(Math.pow(2, currVal));
      }
      return newVal;
    }, []);
    newArr.push(test);
...
let findShortest = (data) => {
  ...
  let shortest = data[0];
  data.map((obj) => {
    if (parseInt(obj.height) < parseInt(shortest.height)) {
      shortest = obj;
    }
  });
